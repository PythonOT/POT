...
def quantized_fused_gromov_wasserstein_partitioned(
    ...
    **kwargs,
    ...
    .. note:: The API changed in version 0.9.8 requesting `part1` and `part2`
        as generated by `get_graph_partition` as inputs when `build_OT=True`
        to fix inconsistencies in rows and columns ordering of the returned OT plan.
...
def get_graph_partition(
    ...
    .. note:: The API changed in version 0.9.8 outputting a list of node indices
        per cluster instead of a single vector of node assignment to match the
        new API of `quantized_fused_gromov_wasserstein_partitioned`.
...
def get_partition_and_representants_samples(
    ...
    .. note:: The API changed in version 0.9.8 outputting a list of node indices
        per cluster in `part`, instead of a single vector of node assignment, to match the
        new API of `quantized_fused_gromov_wasserstein_partitioned`.
