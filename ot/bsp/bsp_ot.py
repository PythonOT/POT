...
def bsp_solve(X, Y, n_plans=64, lp_power=2, initial_plan=None):
    ...
    if initial_plan is not None:
        ...
        if initial_plan.shape != (X.shape[0],):
            raise ValueError("initial_plan must have shape (n,) where n is the number of points")
    ...
    # take lp root
    # if lp_power != -1:
        # cost = cost ** (1/lp_power)

    # add warning if cost_inner and cost differ significantly (due to numerical issues)
    if not np.isclose(cost, cost_inner, rtol=1e-5):
        warnings.warn("Cost computed from plan differs from cost returned by solver. This may be due to numerical issues. Cost from plan: {}, Cost from solver: {}".format(cost, cost_inner))
...
def merge_bijections(X, Y, plans, lp_power=2):
    ...
    cost = nx.mean((X - Y[plan])**lp_power)
    # take lp root
    if lp_power != -1:
        cost = cost ** (1/lp_power)
