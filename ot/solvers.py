    ...
    if reg is None or reg == 0:  # exact OT
        if unbalanced is None:  # Exact balanced OT
            ...
            if unbalanced_type.lower() == "kl":
                value = value_linear + unbalanced * (
                    nx.kl_div(nx.sum(plan, 1), a) + nx.kl_div(nx.sum(plan, 0), b)
                )
            else:
                ...
                value = (
                    value_linear
                    + unbalanced * nx.sum(err_a**2)
                    + unbalanced * nx.sum(err_b**2)
                )
            ...
            plan, log = partial_wasserstein_lagrange(
                a, b, M, reg_m=unbalanced**2, log=True, numItermax=max_iter
            )

            value_linear = nx.sum(M * plan)
            err_a = nx.sum(plan, 1) - a
            err_b = nx.sum(plan, 0) - b
            value = value_linear + nx.sqrt(
                unbalanced**2 / 2.0 * (nx.sum(nx.abs(err_a)) + nx.sum(nx.abs(err_b)))
            )
        ...
        if unbalanced is None:  # Balanced regularized OT
            ...
            if (
                ...
                value = (
                    value_linear
                    + reg * nx.kl_div(plan, a[:, None] * b[None, :])
                    + unbalanced
                    * (nx.kl_div(nx.sum(plan, 1), a) + nx.kl_div(nx.sum(plan, 0), b))
                )
                ...
                plan, log = lbfgsb_unbalanced(
                    ...
                    reg_m=unbalanced,
                    reg_div=reg_type,
                    regm_div=unbalanced_type.lower(),
                    numItermax=max_iter,
                    stopThr=tol,
                    verbose=verbose,
                    log=True,
    ...
    if reg is None or reg == 0:  # exact OT
        if unbalanced is None and unbalanced_type.lower() not in [
            ...
            if M is None:  # Partial Gromov-Wasserstein problem
                ...
                if loss.lower() != "l2":
                    raise (
                        NotImplementedError("Partial GW only implemented with L2 loss")
                    )
                if symmetric is not None:
                    raise (
                        NotImplementedError(
                            "Partial GW only implemented with symmetric=True"
                        )
                    )
