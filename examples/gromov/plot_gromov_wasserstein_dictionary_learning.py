...
for idx_atom, atom in enumerate(Cdict_learned):
    ...
    pl.title('(graph) atom ' + str(idx_atom + 1))
...
# %%
# =============================================================================
# Infer a Fused Gromov-Wasserstein dictionary from the dataset of attributed graphs
# =============================================================================
np.random.seed(0)
ps = [ot.unif(C.shape[0]) for C in dataset]
...
for cluster in range(nlabels):
    ...
    if cluster == 0:
        ...
        pl.scatter(unmixings2D[start:end, 0], unmixings2D[start:end, 1], c='C' + str(cluster), marker='o', s=30., label='%s clusters' % (cluster + 1))
