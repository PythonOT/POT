...
###############################################################################
# Spherical Sliced Wasserstein for different seeds and number of samples
# --------------------------------------------------------------------------

n_seed = 10
n_projections = 500
n_samples_array = np.logspace(1, 4, 10, dtype=int)
...
for seed in range(n_seed):
    ...
    for i, n_samples in enumerate(n_samples_array):
        ...
        res[seed, i] = ot.sliced_wasserstein_sphere(xs, xt, n_projections=n_projections, seed=seed, p=1)
...
pl.figure(2)
pl.plot(n_samples_array, res_mean, label=r"$SSW_1$")
pl.fill_between(n_samples_array, res_mean - 2 * res_std, res_mean + 2 * res_std, alpha=0.5)
