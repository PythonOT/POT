...
X1 = np.cos(np.arange(n1) * np.pi / n1)[:, None] + np.cos(
    np.arange(d1) * np.pi / d1)[None, :] + sigma * np.random.randn(n1, d1)
X2 = np.cos(np.arange(n2) * np.pi / n2)[:, None] + np.cos(
    np.arange(d2) * np.pi / d2)[None, :] + sigma * np.random.randn(n2, d2)
