...
$X_{i,j} = \cos(\frac{i}{n_1} \pi) + \cos(\frac{j}{d_1} \pi)$ and $Y_{i,j} = \cos(\frac{i}{n_2} \pi) + \cos(\frac{j}{d_2} \pi)$. The target matrix is then contaminated by adding $5$ row outliers. Intuitively, we expect that the estimated sample distribution should ignore these outliers, i.e. their weights should be zero.
"""
...
X = torch.cos(torch.arange(n1) * torch.pi /
              n1)[:, None] + torch.cos(torch.arange(d1) * torch.pi / d1)[None, :]
# generate clean target data mixed with outliers
Y_noisy = torch.randn((n, d2)) * 10.0
Y_noisy[:n2, :] = torch.cos(torch.arange(
    n2) * torch.pi / n2)[:, None] + torch.cos(torch.arange(d2) * torch.pi / d2)[None, :]
Y = Y_noisy[:n2, :]

X, Y_noisy, Y = X.double(), Y_noisy.double(), Y.double()

fig, ax = pl.subplots(nrows=1, ncols=3, figsize=(12, 5))
ax[0].imshow(X, vmin=-2, vmax=2)
ax[0].set_title('$X$')

ax[1].imshow(Y, vmin=-2, vmax=2)
ax[1].set_title('Clean $Y$')

ax[2].imshow(Y_noisy, vmin=-2, vmax=2)
ax[2].set_title('Noisy $Y$')
...
for i in range(niter):
    ...
    loss = coot2(X, Y_noisy, py=(b, None))
...
print("Marginal distribution = {}".format(b.detach().numpy()))
...
pi_sample, pi_feature = coot(X, Y_noisy, py=(b, None), log=False, verbose=True)
