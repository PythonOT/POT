...
the CO-Optimal Transport distance [47] between two matrices. More precisely, given a source data
:math:`(X, \mu_x^{(s)}, \mu_x^{(f)})` and a target matrix :math:`Y` associated with a fixed histogram on features
:math:`\mu_y^{(f)}`, we want to solve the following problem
...
allows us to compute the CO-Optimal Transport distance with :any:`ot.coot.co_optimal_transport2`
