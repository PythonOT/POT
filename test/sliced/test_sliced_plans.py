...
def test_min_pivot_sliced():
    ...
    _, min_cost = ot.sliced.min_sliced_transport_plan(x, x, a, a, n_projections=10)
    np.testing.assert_almost_equal(min_cost, 0.0)

    _, min_cost = ot.sliced.min_sliced_transport_plan(
    ...
    # test without provided projections
    ot.sliced.min_sliced_transport_plan(
        x, y, a, b, n_projections=n_projections, log=True
    )

    # test with invalid shapes
    with pytest.raises(AssertionError):
        ot.sliced.min_sliced_transport_plan(x[:, 1:], y, projections=projections)

    # test the logs
    _, min_cost, log = ot.sliced.min_sliced_transport_plan(
    ...
    ot.sliced.min_sliced_transport_plan(
        x, y, projections=projections, metric="minkowski"
    )
    ot.sliced.min_sliced_transport_plan(
        x, y, projections=projections, metric="euclidean"
    )
    ot.sliced.min_sliced_transport_plan(
        x, y, projections=projections, metric="cityblock"
    )

    # test with an unsupported metric
    with pytest.raises(AssertionError):
        ot.sliced.min_sliced_transport_plan(
            x, y, projections=projections, metric="mahalanobis"
        )

    # test with a batch size
    ot.sliced.min_sliced_transport_plan(
        x, y, a, b, projections=projections, batch_size=2
    )
...
def test_expected_sliced():
    ...
    _, expected_cost = ot.sliced.expected_sliced_plan(
    ...
    # test without provided projections
    ot.sliced.expected_sliced_plan(x, y, n_projections=n_projections, log=True)
    ot.sliced.expected_sliced_plan(x, y, a, b, n_projections=n_projections, log=True)
    ...
    # test the logs
    _, min_cost, log = ot.sliced.expected_sliced_plan(
        x, y, a, b, projections=projections, dense=False, log=True
    )
    assert len(log) == 4
    costs = log["costs"]
    ...
    # test with the minkowski metric
    ot.sliced.expected_sliced_plan(x, y, projections=projections, metric="minkowski")

    # test with an unsupported metric
    with pytest.raises(AssertionError):
        ot.sliced.expected_sliced_plan(
            x, y, projections=projections, metric="mahalanobis"
        )
...
def test_sliced_plans_backends(nx):
    ...
    _, expected_cost_b = ot.sliced.expected_sliced_plan(
    ...
    # for min_sliced_transport_plan
    _, min_cost_b = ot.sliced.min_sliced_transport_plan(
        x_b, y_b, a_b, b_b, dense=True, projections=projections_b
    )
    # result should be the same than numpy version
    _, min_cost = ot.sliced.min_sliced_transport_plan(
    ...
    # test with the minkowski metric
    ot.sliced.min_sliced_transport_plan(
        x_b, y_b, projections=projections_b, metric="minkowski"
    )
