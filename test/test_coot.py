...
def test_coot(nx):
    ...
    np.testing.assert_allclose(pi_sample_torch, nx.from_numpy(np.flipud(id_sample).copy()), atol=1e-04)
    np.testing.assert_allclose(pi_feature_torch, nx.from_numpy(id_feature.copy()), atol=1e-04)
...
def test_entropic_coot(nx):
    ...
    pi_sample_torch, pi_feature_torch = coot(X=xs_torch, Y=xt_torch, eps=eps, nits_ot=nits_ot)

    np.testing.assert_allclose(pi_sample_torch, nx.from_numpy(pi_sample.copy()), atol=1e-04)
    np.testing.assert_allclose(pi_feature_torch, nx.from_numpy(pi_feature.copy()), atol=1e-04)
...
def test_fused_coot(nx):
    ...
    pi_sample_torch, pi_feature_torch = coot(X=xs_torch, Y=xt_torch, alpha=alpha, D=D_torch)

    np.testing.assert_allclose(pi_sample, id_sample, atol=1e-04)
    np.testing.assert_allclose(pi_feature, id_feature, atol=1e-04)
    np.testing.assert_allclose(pi_sample_torch, nx.from_numpy(id_sample), atol=1e-04)
    np.testing.assert_allclose(pi_feature_torch, nx.from_numpy(id_feature), atol=1e-04)
