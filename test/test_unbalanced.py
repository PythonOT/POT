...
@pytest.mark.parametrize(
    "method,reg_type",
    itertools.product(
        ["sinkhorn", "sinkhorn_stabilized", "sinkhorn_reg_scaling"], ["kl", "entropy"]
    ),
)
...
@pytest.mark.parametrize(
    "method,reg_type, log",
    itertools.product(
        ["sinkhorn", "sinkhorn_stabilized", "sinkhorn_reg_scaling"],
        ["kl", "entropy"],
        [True, False],
    ),
)
...
@pytest.mark.parametrize(
    "method,reg_m",
    itertools.product(
        ["sinkhorn", "sinkhorn_stabilized", "sinkhorn_reg_scaling"], [1, float("inf")]
    ),
)
...
@pytest.mark.parametrize(
    "reg_div,regm_div", itertools.product(["kl", "l2", "entropy"], ["kl", "l2"])
)
def test_lbfgsb_unbalanced(nx, reg_div, regm_div):
    ...
    G, log = ot.unbalanced.lbfgsb_unbalanced(
        a, b, M, 1, 10, reg_div=reg_div, regm_div=regm_div, log=True, verbose=False
    )

    ab, bb, Mb = nx.from_numpy(a, b, M)

    Gb, log = ot.unbalanced.lbfgsb_unbalanced(
        ab, bb, Mb, 1, 10, reg_div=reg_div, regm_div=regm_div, log=True, verbose=False
    )
...
def test_lbfgsb_unbalanced_relaxation_parameters(nx, reg_div, regm_div):
    ...
    G = ot.unbalanced.lbfgsb_unbalanced(
        a,
        b,
        M,
        1,
...
def test_lbfgsb_reference_measure(nx, reg_div, regm_div):
    ...
    G, _ = ot.unbalanced.lbfgsb_unbalanced(
        a,
        b,
        M,
        reg=1,
    ...
    G0, _ = ot.unbalanced.lbfgsb_unbalanced(
        a,
        b,
        M,
        reg=1,
...
def test_mm_convergence(nx, div):
    ...
    G, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, div=div, verbose=False, log=True
    )
    loss = nx.to_numpy(
...
def test_mm_relaxation_parameters(nx, div):
    ...
    G0, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, reg=reg, div=div, verbose=False, log=True
    )
    loss_0 = nx.to_numpy(
        ot.unbalanced.mm_unbalanced2(
            a, b, M, reg_m=reg_m, reg=reg, div=div, verbose=True
        )
    )

    for opt in list_options:
        G1, _ = ot.unbalanced.mm_unbalanced(
            a, b, M, reg_m=opt, reg=reg, div=div, verbose=False, log=True
        )
        loss_1 = nx.to_numpy(
            ot.unbalanced.mm_unbalanced2(
                a, b, M, reg_m=opt, reg=reg, div=div, verbose=True
            )
        )
...
def test_mm_reference_measure(nx, div):
    ...
    G0, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, c=None, reg=reg, div=div, verbose=False, log=True
    )
    loss_0 = ot.unbalanced.mm_unbalanced2(
        a, b, M, reg_m=reg_m, c=None, reg=reg, div=div, verbose=True
    )
    loss_0 = nx.to_numpy(loss_0)

    G1, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, c=c, reg=reg, div=div, verbose=False, log=True
    )
    loss_1 = ot.unbalanced.mm_unbalanced2(
        a, b, M, reg_m=reg_m, c=c, reg=reg, div=div, verbose=True
    )
...
def test_mm_wrong_divergence(nx):
    ...
    G0, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, reg=reg, div="kl", verbose=False, log=True
    )
    loss_0 = nx.to_numpy(
        ot.unbalanced.mm_unbalanced2(
            a, b, M, reg_m=reg_m, reg=reg, div="kl", verbose=True
        )
    )

    G1, _ = ot.unbalanced.mm_unbalanced(
        a, b, M, reg_m=reg_m, reg=reg, div="wrong_div", verbose=False, log=True
    )
    loss_1 = nx.to_numpy(
        ot.unbalanced.mm_unbalanced2(
            a, b, M, reg_m=reg_m, reg=reg, div="wrong_div", verbose=True
        )
    )
