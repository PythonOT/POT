...
def test_gromov(nx):
    ...
    for armijo in [False, True]:
        gw, log = ot.gromov.gromov_wasserstein2(
            C1, C2, None, q, "kl_loss", armijo=armijo, log=True
        )
        gwb, logb = ot.gromov.gromov_wasserstein2(
            C1b, C2b, pb, None, "kl_loss", armijo=armijo, log=True
        )
        gwb = nx.to_numpy(gwb)

        gw_val = ot.gromov.gromov_wasserstein2(
            C1, C2, p, q, "kl_loss", armijo=armijo, G0=G0, log=False
        )
        gw_valb = nx.to_numpy(
            ot.gromov.gromov_wasserstein2(
                C1b, C2b, pb, qb, "kl_loss", armijo=armijo, G0=G0b, log=False
            )
        )
...
def test_asymmetric_gromov(nx):
    ...
    G, log = ot.gromov.gromov_wasserstein(
        C1, C2, p, q, "square_loss", G0=G0, log=True, symmetric=False, verbose=True
    )
    Gb, logb = ot.gromov.gromov_wasserstein(
        C1b, C2b, pb, qb, "square_loss", log=True, symmetric=False, G0=G0b, verbose=True
    )
    Gb = nx.to_numpy(Gb)
    # check constraints
    np.testing.assert_allclose(G, Gb, atol=1e-06)
    ...
    np.testing.assert_allclose(log["gw_dist"], 0.0, atol=1e-04)
    np.testing.assert_allclose(logb["gw_dist"], 0.0, atol=1e-04)
...
def test_gromov_integer_warnings(nx):
    ...
    # check constraints
    np.testing.assert_allclose(G, Gb, atol=1e-06)
    np.testing.assert_allclose(G, 0.0, atol=1e-09)
...
def test_gromov_dtype_device(nx):
    ...
    for tp in nx.__type_list__:
        ...
        with warnings.catch_warnings():
            ...
            Gb = ot.gromov.gromov_wasserstein(
                C1b, C2b, pb, qb, "square_loss", G0=G0b, verbose=True
            )
            gw_valb = ot.gromov.gromov_wasserstein2(
                C1b, C2b, pb, qb, "kl_loss", armijo=True, G0=G0b, log=False
            )
...
def test_gromov_device_tf():
    ...
    if len(tf.config.list_physical_devices("GPU")) > 0:
        ...
        gw_valb = ot.gromov.gromov_wasserstein2(
            C1b, C2b, pb, qb, "kl_loss", armijo=True, log=False
        )
        nx.assert_same_dtype_device(C1b, Gb)
        nx.assert_same_dtype_device(C1b, gw_valb)
...
def test_gw_helper_backend(nx):
    ...
    C1b, C2b, pb, qb, G0b = nx.from_numpy(C1, C2, p, q, G0)
    Gb, logb = ot.gromov.gromov_wasserstein(
        C1b, C2b, pb, qb, "square_loss", armijo=False, symmetric=True, G0=G0b, log=True
    )

    # calls with nx=None
    constCb, hC1b, hC2b = ot.gromov.init_matrix(
        C1b, C2b, pb, qb, loss_fun="square_loss"
    )
    ...
    def line_search(cost, G, deltaG, Mi, cost_G):
        return ot.gromov.solve_gromov_linesearch(
            G, deltaG, cost_G, C1b, C2b, M=0.0, reg=1.0, nx=None
        )
...
def test_fgw_helper_backend(nx):
    ...
    def line_search(cost, G, deltaG, Mi, cost_G):
        return ot.gromov.solve_gromov_linesearch(
            G, deltaG, cost_G, C1b, C2b, M=(1 - alpha) * Mb, reg=alpha, nx=None
        )
    ...
    # feed the precomputed local optimum Gb to cg
    res_armijo, log_armijo = ot.optim.cg(
        pb,
        qb,
        (1 - alpha) * Mb,
        alpha,
...
def test_fgw_barycenter(nx):
    ...
    recovered_Xb = ot.gromov.update_feature_matrix(
        lambdas, [y.T for y in Ysb], logb["Ts_iter"][-1], pb
    ).T
