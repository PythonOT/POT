...
def test_semirelaxed_gromov(nx):
    ...
    C2 = np.array([[0.8, 0.05], [0.05, 1.0]], dtype=np.float64)
    for i in range(nt):
        for j in range(nt):
...
def test_semirelaxed_fgw(nx):
    ...
    for loss_fun in ["square_loss", "kl_loss"]:
        ...
        srgw_ = ot.gromov.semirelaxed_fused_gromov_wasserstein2(
            M, C1, C2, p, loss_fun=loss_fun, alpha=0.5, symmetric=True, log=False, G0=G0
        )
...
def test_entropic_semirelaxed_fgw(nx):
    ...
    for loss_fun in ["square_loss", "kl_loss"]:
        ...
        srgw_ = ot.gromov.entropic_semirelaxed_fused_gromov_wasserstein2(
            M,
            C1,
            C2,
            p,
