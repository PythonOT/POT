...
def test_bsp_ot_plan_merge_decrease():
    ...
    assert cost <= cost_plan_1, "Cost of merged plan is not smaller than one of the input plans"
    assert cost <= cost_plan_2, "Cost of merged plan is not smaller than one of the input plans"
...
def test_bsp_ot_relative_error():
    ...
    assert relative_error < 0.1, "Relative error is too large: {}".format(relative_error)
